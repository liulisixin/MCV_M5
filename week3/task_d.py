...
def generate_gt_for_validation(dataset, dataset_name):
    for _, img_file in dataset:
        gt_filename = '{}.txt'.format(img_file[:-4])
        if os.path.exists('./gt_{}/{}'.format(dataset_name, gt_filename)):
            lines = []
            with open('./gt_{}/{}'.format(dataset_name, gt_filename)) as f:
                for line in f:
                    gt_file_content = line.split(' ')
                    ...
                    if category_id == 1:
                        ...
                        category = 'Car'
                    format_line = write_detection_format(category, bbox)
                    lines.append(format_line)                
            with open('./validation/label_2/{}'.format(gt_filename), 'w') as validation_file:
                for line in lines[:-1]:
                    validation_file.write(' '.join(line) + '\n')
                validation_file.write(' '.join(lines[-1]))
                
    with open('./validation/gt.txt', 'w') as gt_f:
        sorted_label_2 = sorted(os.listdir('./validation/label_2'))
        for idx, f in enumerate(sorted_label_2):
            if idx + 1 < len(sorted_label_2):
                gt_f.write('/home/grupo06/ruben/MCV_M5/week3/validation/label_2/{}\n'.format(f))
            else:
                gt_f.write('/home/grupo06/ruben/MCV_M5/week3/validation/label_2/{}'.format(f))
...
def write_detection_format(class_str, box, score=None):
    ...
    line.append('-1')
    line.append('-10')
    line.append(str(box[0]))
    line.append(str(box[1]))
    line.append(str(box[2]))
    line.append(str(box[3]))
    line.append('-1')
    line.append('-1')
    line.append('-1')
    line.append('-1000')
...
def generate_model(config):
    ...
    #generate_gt_for_validation(validation, config['task_d']['dataset_train'])
    cfg, trainer = train_model(config)
    validate_model(config, cfg, validation, trainer)
...
def validate_model(config, cfg, validation_dataset, trainer):
    ...
    for img_filename, img_file in validation_dataset:
        ...
        gt_filename = img_file.replace(".png", ".txt")

        if os.path.exists('./gt_{}/{}'.format(config['task_d']['dataset_train'], gt_filename)):        
            filename = '{}/{}'.format('./detection/data', gt_filename)
            f = open(filename, 'w')
    ...
    kitti_metadata = MetadataCatalog.get('{}_validation'.format(config['task_d']['dataset_train']))
    coco_validation_format = register_dataset(validation_dataset, config['task_d']['dataset_train'], config)
    """
    for idx, image_file in enumerate(random.sample(coco_validation_format, 10)):
        im = cv2.imread(image_file['file_name'])
        outputs = predictor(im)
        v = Visualizer(im[:, :, ::-1], metadata=kitti_metadata, scale=0.8)
        v = v.draw_instance_predictions(outputs["instances"].pred_classes.to("cpu"))
        cv2.imwrite('example_{}.png'.format(idx), v.get_image()[:, :, ::-1])
    """
    evaluator = COCOEvaluator("{}_validation".format(config['task_d']['dataset_train']), cfg, False, output_dir="./output/")
    val_loader = build_detection_test_loader(cfg, '{}_validation'.format(config['task_d']['dataset_train']))
...
def cleanup():
    ...
    for f in glob.glob('./gt_MOTSChanllenge' + '**/*.txt'):
        os.remove(f)
    for f in glob.glob('./validation/label_2' + '**/*.txt'):
        os.remove(f)
